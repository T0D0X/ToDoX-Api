...
class TodoServiceTest extends AnyFlatSpec with Matchers with MockFactory {
  ...
  "update" should "Success result" in new Testing {
    ...
    todoRepository.updateTodoItem
      .expects(todoItem.id, todoUpdatePriority)
      .returns(ZIO.unit)
  ...
  "getByUserId" should "Success operation" in new Testing {
    todoRepository.getAllByUserId
      .expects(*)
      .returns(ZIO.succeed(List(todoItem)))
  ...
  trait Testing {
    ...
    val todoCreate =
      generateUnsafe[CreateTodoRequest].copy(userId = todoItem.userId)
